        ...
        stage('Deploy') {
            steps {
                ...
                sshagent(credentials: ['mudit_test']) {
                    sh "sudo scp -o StrictHostKeyChecking=no target/*.jar ${DEPLOY_USER}@${DEPLOY_SERVER}:${REMOTE_DIR}"
                    sh "sudo ssh ${DEPLOY_USER}@${DEPLOY_SERVER} 'cd ${REMOTE_DIR} && java -jar *.jar'"
                }
            }
        }
    }
